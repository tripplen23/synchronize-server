...
namespace Ecommerce.Service.src.Service
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<CategoryReadDto> CreateCategoryAsync(CategoryCreateDto categoryCreateDto)
        {
            ...
            {
                // validations
                if (string.IsNullOrEmpty(categoryCreateDto.CategoryName)) throw AppException.InvalidInputException("Category name cannot be empty");
                if (categoryCreateDto.CategoryName.Length > 20) throw AppException.InvalidInputException("Category name cannot be longer than 20 characters");

                string imagePatten = @"^.*\.(jpg|jpeg|png|gif|bmp)$";
                Regex imageRegex = new(imagePatten);
                if (categoryCreateDto.CategoryImage is not null && !imageRegex.IsMatch(categoryCreateDto.CategoryImage)) throw AppException.InvalidInputException("Category image can only be jpg|jpeg|png|gif|bmp");
        ...
        public async Task<CategoryReadDto> UpdateCategoryByIdAsync(Guid categoryId, CategoryUpdateDto categoryUpdateDto)
        {
            try
            {
                ...
                if (categoryUpdateDto.CategoryName is not null && categoryUpdateDto.CategoryName.Length > 20) throw AppException.InvalidInputException("Category name cannot be longer than 20 characters");

                string imagePatten = @"^.*\.(jpg|jpeg|png|gif|bmp)$";
                Regex imageRegex = new(imagePatten);
                if (categoryUpdateDto.CategoryImage is not null && !imageRegex.IsMatch(categoryUpdateDto.CategoryImage)) throw AppException.InvalidInputException("Category image can only be jpg|jpeg|png|gif|bmp");

                foundCategory.Name = categoryUpdateDto.CategoryName ?? foundCategory.Name;
                foundCategory.Image = categoryUpdateDto.CategoryImage ?? foundCategory.Image;
